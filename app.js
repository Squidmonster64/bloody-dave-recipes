...
function heroAssetUrl(recipeId) {
  return `assets/hero/${recipeId}.jpg?v=${HERO_ASSET_VERSION}`;
}

function applyHeroFocal(node, recipe) {
  if (!node || !recipe?.heroFocal) return;
  node.style.setProperty('--hero-focal', recipe.heroFocal);
}
...
function normaliseRecipe(raw, index = 0, archive = false, localPromoted = false) {
  ...
  const defaultHeroImage = !archive && /^BD-\d{4,}$/.test(stableId) ? heroAssetUrl(stableId) : '';
  ...
  const heroFocal = firstDefined(raw.heroFocal, raw.hero_focal, raw.objectPosition, HERO_FOCAL[stableId], '50% 50%').toString();
...
function applyCardImage(node, recipe) {
  ...
  const wrap = $('.card-image-wrap', node);
...
function detailHero(recipe) {
  if (recipe.heroImage) {
    return `<img class="detail-hero" src="${escapeHtml(recipe.heroImage)}" alt="${escapeHtml(recipe.heroImageSubject || recipe.title)}" style="--hero-focal:${escapeHtml(recipe.heroFocal || '50% 50%')}">`;
  }
